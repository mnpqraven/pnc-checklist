...
use crate::{
    algorithm::types::*,
    prisma::{loadout, unit_skill},
    requirement::types::PrismaData,
    service::db::get_db,
    stats::types::*,
    traits::FromAsync,
    unit::types::*,
...
impl FromAsync<loadout::Data> for ILoadout {
    async fn from_async(value: loadout::Data) -> Self {
        ...
        .await;
        Self {
            skill_level: IUnitSkill::from_async(value.skill_level().unwrap().unwrap().clone())
                .await,
...
impl PrismaData for unit_skill::Data {
    async fn get_unit_id(&self) -> Option<String> {
        let client = get_db().await;
        client
            .loadout()
            .find_unique(loadout::id::equals(self.loadout_id.clone()))
            .exec()
            .await
            .unwrap()
            .map(|loadout| loadout.unit_id)
    }
    ...
    async fn get_loadout_type(&self) -> Option<LoadoutType> {
        ...
        client
            ...
            .await
            .unwrap()
            .map(|loadout| LoadoutType::from_str(&loadout.loadout_type).unwrap())
    }
}
