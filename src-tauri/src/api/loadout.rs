...
use crate::{
    loadout::get_loadout_tuple,
    prisma::loadout,
    requirement::types::{LevelRequirement, TRequirement},
};
    ...
    Router::<Ctx>::new()
        ...
        .mutation("save", |t| {
            t(|ctx, loadouts: Vec<loadout::Data>| async move {
                ...
                // be UnitRequirement
                LevelRequirement::calculate(loadouts.clone(), None, true, None).await?; // early return if calculation failed with given transaction
                ctx.client
